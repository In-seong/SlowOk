...
- **SlowOK_App/**: 모바일 앱 (Android Kotlin / iOS Swift WebView 래퍼, user/admin 2개 product flavor)
...
### 역할 시스템 (4종)
- **USER**: 학습자/학부모 — 사용자 앱 접근
- **ADMIN**: 기관 관리자 — 관리자 앱 접근, 소속 기관 데이터만
- **MASTER**: 슈퍼 관리자 — 모든 기관/권한 접근 (institution_id가 null), `isMaster()` 메서드로 판별
- **TEST**: 테스트용 계정
...
### 미들웨어 별칭 (bootstrap/app.php)
- `role` → CheckRole, `perm` → CheckPermission, `institution` → InjectInstitutionScope
- `active-profile` → InjectActiveProfile, `feature` → CheckFeature
...
- MASTER 역할은 모든 권한을 자동 보유 (`hasPermission()` 메서드에서 처리)

### 기능 플래그 시스템
- `CheckFeature` 미들웨어로 기관/사용자별 기능 게이팅
- `FeatureGateService` → Plan + InstitutionPlan + UserFeatureOverride 조합으로 접근 제어
- 기관 요금제(Plan)에 따라 사용 가능한 기능이 달라짐

## TypeScript 코드 작성 규칙
- **strict null check 준수**: 배열 인덱스 접근(`arr[i]`)은 반드시 `undefined` 가능성을 처리 (가드 또는 optional chaining)
- **미사용 import/파라미터 금지**: `noUnusedLocals: true`, `noUnusedParameters: true` 설정 — 코드 작성/수정 후 반드시 확인
...
## 배포
- `deploy/deploy.sh`: 프론트엔드 빌드 → rsync로 백엔드 동기화 → composer install → 마이그레이션 → 캐시 최적화 → nginx 재시작
- `deploy/nginx.conf`: 듀얼 도메인 (user/admin), SSL, PHP-FPM, SPA 폴백 설정
- 프론트엔드 빌드 산출물은 `slowok_back/public/user/`, `slowok_back/public/admin/`으로 출력됨
