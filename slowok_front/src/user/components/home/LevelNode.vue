...
<template>
  <div class="flex flex-col items-center gap-1" style="width: 140px;">
    <!-- Title (상단) -->
    <div
      class="text-[11px] leading-snug text-center w-full line-clamp-2 px-2 py-1 rounded-lg mb-0.5"
      :class="status === 'locked' ? 'text-[#BDBDBD] font-medium bg-transparent' : 'text-[#2E7D32] font-bold bg-white/30 border border-white/60 backdrop-blur-sm shadow-sm'"
    >
      {{ challenge.title }}
    </div>
    ...
    <button
      ...
      class="relative w-20 h-20 rounded-full flex items-center justify-center transition-all"
      :class="[
        status === 'completed'
    ...
    >
      ...
      <!-- Current: Star icon -->
      <svg v-else-if="status === 'current'" class="w-8 h-8 text-white" viewBox="0 0 24 24" fill="currentColor">
        <path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
      </svg>

      <!-- Locked: Lock -->
      <svg v-else class="w-7 h-7 text-[#9E9E9E]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
