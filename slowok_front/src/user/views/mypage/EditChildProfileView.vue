...
onMounted(() => {
  const child = authStore.profiles.find(p => p.profile_id === childId.value)
  if (child) {
    form.value.name = child.decrypted_name || child.name || ''
    form.value.birth_date = child.birth_date || ''
    form.value.phone = child.decrypted_phone || child.phone || ''
  }
})
...
async function handleSubmit(): Promise<void> {
  ...
  const success = await authStore.updateChildProfile(childId.value, data)
  if (success) {
    successMessage.value = '자녀 프로필이 수정되었습니다.'
  }
}
</script>
